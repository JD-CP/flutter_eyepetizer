class TabInfoEntity {
	...
	TabInfoEntity({this.tabInfo});

	TabInfoEntity.fromJson(Map<String, dynamic> json) {
		tabInfo = json['tabInfo'] != null ? new TabInfo.fromJson(json['tabInfo']) : null;
	}
...
class TabInfo {
	...
	TabInfo.fromJson(Map<String, dynamic> json) {
		if (json['tabList'] != null) {
			tabList = new List<TabInfoItem>();(json['tabList'] as List).forEach((v) { tabList.add(new TabInfoItem.fromJson(v)); });
		}
...
class TabInfoItem {
	...
	TabInfoItem({this.nameType, this.apiUrl, this.name, this.tabType, this.id, this.adTrack});
